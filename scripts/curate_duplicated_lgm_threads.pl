...
my $fh = FileHandle->new('/Users/anja/Documents/G2P/other_DD.tsv', 'w');
print $fh join("\t", qw/locus genotype-mechanism disease_a disease_b publication_list_a publication_count_a publication_list_b publication_count_b publication_similarity phenotype_list_a phenotype_count_a phenotype_list_b phenotype_count_b phenotype_similarity/), "\n";
foreach my $list (@{$merge_list}) {
  ...
  for (my $i=0; $i < scalar(@{$sets}); $i++ ) {
    for (my $j=$i+1; $j < scalar(@{$sets}); $j++) {
      ...
      my $publication_list_b =  join(';', sort  @{$set_b->{'publications'}});
      ...
      my $phenotype_list_b =  join(';', sort  @{$set_b->{'phenotypes'}});

      my $gene_symbol = $set_a->{gene_symbol};
      my $actions = join(',', @{$set_a->{actions}});
      if (grep {$actions eq $_} ('biallelic all missense/in frame', 'biallelic uncertain', 'hemizygous loss of function', 'hemizygous uncertain', 'imprinted loss of function', 'mosaic activating')) {
        print $fh join("\t", $gene_symbol, $actions, $set_a->{'disease_name'}, $set_b->{'disease_name'}, $publication_list_a, $size_publication_a, $publication_list_b, $size_publication_b, $publication_similarity, $phenotype_list_a, $size_phenotype_a, $phenotype_list_b,  $size_phenotype_b, $phenotype_similarity), "\n"; 
      }
    }
  }
}
